...
| FR-7 | Web UI: per-mirror card carrying a delay slider, a preview toggle (live/delayed) and a full-buffer download button (fixed length = that mirror's `capacity_s`; the API keeps free-form `seconds`); status on the settings page | UI-1..11 + API contract test |
