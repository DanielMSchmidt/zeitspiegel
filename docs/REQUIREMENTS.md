...
| FR-9 | Config file for boot defaults incl. camera controls (focus pinning, exposure); runtime changes via API. A control the device does not implement is **skipped and reported**, not fatal — fixed-focus cameras (E-9) expose no focus controls at all. Any other control failure still aborts the open | Restart test; UT-26; control test vs v4l2loopback/device |
...
| FR-15 | Role election, dynamic: on boot a unit looks for the shared `zeitspiegel` network and joins it, or hosts it if nobody does — units are added to and removed from an installation at any time, always by cutting power, and there is no configured fleet size anywhere. If the host disappears exactly one survivor takes over within seconds; a returning ex-host rejoins as an ordinary member without preempting; a host serving anyone (a member unit or just a guest's phone) never takes the network down | UT-19, IT-11, ST-7/9/10/11/13 |
| FR-16 | Displayed image preserves the camera's aspect ratio: fitted and centred within the panel, remainder letterboxed/pillarboxed in black, never stretched. A mirror used to judge body line must not distort proportion | UT-27 + manual visual check |

## 2. Non-functional requirements

| ID | Requirement |
|---|---|
| NFR-1 | **1080p@30 sustained** (default). `auto` opens the largest MJPEG mode ≤1080p that sustains ~30 fps, trading resolution away to hold the rate rather than the reverse — a mode's advertised frame interval is read, never assumed (E-2, UT-28). `720p60`/`1080p30` remain selectable, and a camera offering only 60 fps is opened at 60. Drop rate < 0.1 % over 24 h |
...
| NFR-10 | Discoverable as `zeitspiegel.local` (mDNS) on the appliance's own AP; fallback address `http://10.42.0.1` (the AP gateway). Whichever unit is hosting answers to `zeitspiegel.local` and takes `10.42.0.1`, so both stay valid across a failover; the others answer to `zeitspiegel-<unit id>.local`. Hostnames are set transiently (`hostnamectl --transient`) because `/etc/hostname` is read-only once the overlay is sealed, and announced via `avahi-set-host-name` — Avahi does not follow kernel hostname changes, and the live rename is what moves `zeitspiegel.local` to the new host after a failover |
...
The `/api/v1/peers` routes are always on: a lone appliance answers
`{"peers":[]}`, because "no peers yet" is an ordinary state in an
installation that grows one unit at a time. All API responses carry
`Access-Control-Allow-Origin: *` and `OPTIONS` is answered with 204 (NFR-6).
The endpoint is as unauthenticated as the rest of the control API — anyone
on the open LAN can register or replace a card entry, which adds nothing to
what NFR-6 already accepts, and is noted here so the trade is a recorded
one.
...
| E-2 | *Revised twice.* Default profile `auto`: the camera adapter enumerates every MJPEG mode **with the frame rate each one sustains** and opens the largest that reaches ~30 fps, capped at 1080p (`config.MaxAuto{Width,Height}`) so software decode stays within the Pi 5 budget. Frame rate is the constraint and resolution is maximised under it — a camera offering 1600×1200@15 and 1280×720@30 opens 720p30, because a mode the device cannot deliver is worse than fewer pixels. Owner chose spatial sharpness (dancers read the screen from across a room) over the original 720p60 preference, so among modes that meet the rate, area wins. Tolerance is one frame, so 30000/1001 (29.97) counts as 30. When *nothing* reaches the target the fastest available mode is opened rather than refusing the device — a choppy mirror beats a black screen. `720p60`/`1080p30` remain selectable. **The rate is measured, not assumed:** the opened mode drives status, gap detection and clip encoding (§3), because encoding a 60 fps capture at a nominal 30 would halve clip playback speed. The engine selects by capture timestamp, so a camera drifting from its advertised rate stays correct regardless |
...
| E-8 | **Self-electing, dynamic fleet**, supplementing E-7 rather than superseding it — a lone unit still behaves exactly as E-7 describes. Every card carries the identical image and nothing about the fleet is configured, not even its size: units are bought and added over the years, and on any day the studio powers on whichever stations it needs, in any order, cutting power to stop them. A unit looks for the shared SSID and joins it, or hosts it if nobody does. There is only ever **one** SSID, so the printed poster, the QR codes and `zeitspiegel.local` stay valid whichever unit is hosting. The design is shaped by one hardware fact: a radio in AP mode cannot scan, so a host can never observe a second host on the same SSID — but it CAN count who is associated to it (`iw station dump` works in AP mode), member units and guests' phones alike. Hence the central rule: **a host serving anybody holds its network unconditionally; only a host serving nobody probes** — drops its AP, scans, joins what it finds or reclaims — after 90 s idle, with exponential backoff, and since nobody is attached the probe kicks no one. A short id-derived stagger (4 slots × 3 s) makes simultaneous cold-boot claims unlikely; when they happen anyway, the empty-audience probe heals the split. The accepted residual: two hosts each serving only phones hold a persistent split — each side fully functional, self-resolving on any power cycle. The combined page talks to each unit directly (NFR-6 CORS) rather than being proxied, keeping clip downloads off a second wireless hop. The display name comes from a file on the FAT32 boot partition, which stays writable when the root overlay is sealed |
| E-9 | **Camera class: wide-angle, fixed-focus, no in-camera AI** (docs/HARDWARE.md). The Razer Kiyo is dropped as reference — discontinued, and its successor is an AI camera. Target ~100-110° diagonal: wider covers the room and, because a fixed-focus lens sits at its hyperfocal distance, gives a sharp range of roughly 0.3-0.4 m to infinity — which is what "everything in view is clear" actually requires. Past ~120° barrel distortion curves body lines and nothing corrects it. 4K is rejected: the Pi 5 can *display* it, but decode is software against a 16.7 ms tick (4K ≈ 9× the pixels of 720p), the 60 s buffer is sized for 1080p30 bitrates, and export downscales to 720p anyway. Auto-framing/bokeh/beauty cameras are excluded outright — they are configured only by Windows/macOS vendor software, so over UVC on Linux you inherit whatever firmware state was saved. Consequence for the adapter: fixed-focus devices implement no focus controls, so an unsupported control is skipped and reported rather than aborting the open (FR-9). Ranked shortlist stays provisional until spike S-2 measures a real device |
