...
| FR-3 | Delay changeable at runtime, 0…`capacity_s`, resolution ≥ 0.1 s. Boot value from `default_delay_s` (default 25 s) | Effective ≤ 1 frame interval after the 200 response |
...
| FR-7 | Web UI: per-mirror card carrying a delay slider, a preview toggle (live/delayed) and a full-buffer download button (fixed length = that mirror's `capacity_s`; the API keeps free-form `seconds`); status on the settings page | UI-1..12 + API contract test |
...
| FR-14 | Several appliances on one network are controlled from one page: one card per mirror, this one included, each carrying that mirror's own delay, preview and clip download; changing one must not affect another. A lone appliance's page is that one card and nothing else: no fleet chrome, nothing on it a single mirror cannot do | UI-1..12; IT-10, ST-8; ST-7 for the listing |
