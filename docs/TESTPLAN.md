...
| UT-36 | support | Deploy artifacts carry what the display path needs at *runtime*: every script that installs the SDL runtime also installs `libegl1`/`libegl-mesa0`/`libgles2`, which SDL's KMSDRM backend dlopens and libsdl2 therefore does not depend on — their absence builds and boots cleanly and only surfaces as "EGL not initialized" on a black screen in a venue; and the unit pins `SDL_VIDEODRIVER=kmsdrm` with `XDG_RUNTIME_DIR` pointing at the tmpfs `RuntimeDirectory` creates, so SDL stops probing wayland/x11 and burying the real error |
