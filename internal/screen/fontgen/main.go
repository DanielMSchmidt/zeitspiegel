...
// Digits use rows 1–9 (cols 1–5); x-height letters use rows 4–9; ascenders
// (d, l) use rows 1–9. Cells are centred (col 0 and col 6 blank).
var glyphs = map[rune][12]byte{
	...
	's': {0x00, 0x00, 0x00, 0x00, 0x3C, 0x02, 0x1C, 0x20, 0x20, 0x1E, 0x00, 0x00},
	' ': {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
	'd': {0x00, 0x20, 0x20, 0x20, 0x3C, 0x22, 0x22, 0x22, 0x22, 0x3C, 0x00, 0x00},
	'e': {0x00, 0x00, 0x00, 0x00, 0x1C, 0x22, 0x3E, 0x02, 0x22, 0x1C, 0x00, 0x00},
	'l': {0x00, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x00, 0x00},
	'a': {0x00, 0x00, 0x00, 0x00, 0x1C, 0x20, 0x3C, 0x22, 0x22, 0x3C, 0x00, 0x00},
	'y': {0x00, 0x00, 0x00, 0x00, 0x22, 0x14, 0x08, 0x08, 0x08, 0x08, 0x00, 0x00},
	'r': {0x00, 0x00, 0x00, 0x00, 0x1A, 0x06, 0x02, 0x02, 0x02, 0x02, 0x00, 0x00},
	'i': {0x00, 0x08, 0x00, 0x00, 0x08, 0x08, 0x08, 0x08, 0x08, 0x08, 0x00, 0x00},
	'n': {0x00, 0x00, 0x00, 0x00, 0x1E, 0x22, 0x22, 0x22, 0x22, 0x22, 0x00, 0x00},
}

// Atlas layout: 20 glyphs, each 14×24 (source 7×12 scaled 2×), white on
// transparent. Matches the order screen/badge.go expects — appended to, never
// reordered, so an atlas and a binary built at different times still agree on
// which cell is which. `rin` is what the warm-up line ("ready in 12s") adds.
const atlasOrder = "0123456789s delayrin"

const (
	cellW   = 14 // 7 source cols × 2
	cellH   = 24 // 12 source rows × 2
	atlasW  = cellW * len(atlasOrder)
	atlasH  = cellH
	srcRows = 12
	srcCols = 7
)
